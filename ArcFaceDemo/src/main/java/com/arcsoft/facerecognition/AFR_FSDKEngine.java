...
public class AFR_FSDKEngine {
        ...
        if(feature != null && data != null) {
            if(feature.mFeatureData == null) {
                ...
                if(size > 0 && this.error.mCode == 0) {
                    this.error.mCode = this.FR_GetFeatures(this.handle.intValue(), feature.mFeatureData);
                }
            } else {
                this.error.mCode = 5;
            }
    ...
    public AFR_FSDKError AFR_FSDK_FacePairMatching(AFR_FSDKFace ref, AFR_FSDKFace input, AFR_FSDKMatching score) {
        if(ref != null && input != null && score != null) {
            if(this.handle.intValue() != 0) {
                score.mScore = this.FR_Recognize(this.handle.intValue(), ref.mFeatureData, input.mFeatureData);
                this.error.mCode = this.FR_GetErrorCode(this.handle.intValue());
            } else {
                this.error.mCode = 5;
            }
    ...
    private AFR_FSDKError AFR_FSDK_UpdateFeature(AFR_FSDKFace in, AFR_FSDKFace out) {
        if(in != null && out != null) {
            if(this.handle.intValue() != 0) {
                this.error.mCode = this.FR_UpdateFeature(this.handle.intValue(), in.mFeatureData, out.mFeatureData);
            } else {
                this.error.mCode = 5;
            }
        } else {
